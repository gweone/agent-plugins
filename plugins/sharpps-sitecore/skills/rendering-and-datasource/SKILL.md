...
description: Use when a rendering's `.cshtml` has wording, validation rules/messages, option lists, or links hardcoded directly in the markup, and it needs to become a real Sitecore-driven component - authors edit it through Content Editor/Experience Editor instead of a developer editing HTML. Covers classifying every literal into a field (wording vs. parameter vs. option-list vs. link), grouping fields into Template Sections, the Shared-field rule for non-translatable parameters, a repeatable-option-list pattern, per-language Standard Values defaults, detecting View Rendering vs. Controller Rendering and wiring Datasource Template/Location fields, the mandatory design-plan approval gate before anything is created in Sitecore, and getting new items into the workspace's serialized item tree afterward. Solution-agnostic guidance. Builds on the `sitecore` skill's creation/serialization mechanics rather than duplicating them - read that skill first if unfamiliar.
---
