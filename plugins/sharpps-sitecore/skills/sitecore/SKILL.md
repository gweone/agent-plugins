---
name: sitecore
description: Use when working on a $solutionpathformat$ Sitecore solution scaffolded from the SharpPS solutionitems template - interacting with a running Sitecore instance (publish, Unicorn sync, site warm-up, migrations, template/rendering/datasource creation via SPE Remoting) via the SharpPS.Shells PowerShell module. Covers module installation without admin rights (SharpPS.Shells + the separate SPE Remoting client module, both need -Scope CurrentUser), the self-signed-cert trust gotcha for New-SPESession (must be set in the same process as the connection), the correct low-level way to create Template/Template Section/Template Field items and compose base templates via Add-BaseTemplate (Add-ItemTemplateSection/Add-ItemTemplateField are not real cmdlets), serializing newly-created items to disk via Unicorn's own Export-UnicornItem when the solution has Unicorn configured for that path (not SPE's plain Export-Item, which writes a different/wrong format) - including resolving the real physicalRootPath, a Rainbow filename-truncation gotcha, and retrieving the written files onto a different machine via Receive-RemoteItem when the instance is genuinely remote - and running this from Claude Code specifically (the auto mode classifier blocks live SPE writes by default; getting past that needs an explicit, user-approved permission entry).
---
