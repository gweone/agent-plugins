...
| `shell-session` | Persistent, shared `shell_session_*` tmux sessions on a SharpPS Shells host — either a local pty or a remote shell over SSH — for interactive CLI wizards, remote logins, and long-running/resumable terminal work (e.g. a remote `claude` session) that a plain `shell_execute` one-shot can't handle. Covers the two transports (`mode="local_pty"` vs `mode="ssh"`, the latter needing an allow-listed `host` and never password auth — only `"none"`/agent or `"private_key"`), the four rules (explicit user confirmation before starting, always pass explicit connection details — `workspace` or `host` — on `shell_session_start`, stay inside the `shell_session_*` tool family for the life of a task, and check `shell_session_list`/`_get`/`_screen` before resuming or writing into a session you didn't start yourself), plus `shell_session_stop`, `shell_session_capabilities`, `shell_claude_sessions_list`, and how to run elevated (`sudo`) commands in a session without writing secrets into it. |
