...
| `pty-sessions` | Persistent, shared `shell_pty_*` tmux sessions on a SharpPS Shells host, for interactive CLI wizards and long-running/resumable terminal work (e.g. a remote `claude` session) that a plain `shell_execute` one-shot can't handle. Covers the four rules: always pass an explicit `workspace` on `shell_pty_start`, plan before acting via `shell_pty_plan_context`, stay inside the `pty_*` tool family for the life of a task so the context log stays a complete audit trail, and populate context (`shell_pty_context`, `shell_pty_screen`) before resuming or writing into a session you didn't start yourself. Also covers `shell_pty_list`, `shell_pty_compact_context`, `shell_pty_stop`, `shell_claude_sessions_list`, and how to run elevated (`sudo`) commands in a pty without writing secrets into the logged context. |
